...
EmotionBarVisualizer::EmotionBarVisualizer(std::vector <std::string> const &emotionNames, cv::Size size,
                                           cv::Size padding,
        ...
        internalPadding(0) {
    auto itMax = std::max_element(emotionNames.begin(), emotionNames.end(),
                                  [](std::string const &lhs, std::string const &rhs) {
                                      return lhs.length() < rhs.length();
                                  });

    textSize = cv::getTextSize(*itMax, cv::FONT_HERSHEY_COMPLEX_SMALL, textScale, textThickness, &textBaseline);
    ystep = (emotionNames.size() < 2) ? 0 : (size.height - 2 * padding.height - textSize.height) /
                                            (emotionNames.size() - 1);
}
...
void EmotionBarVisualizer::draw(cv::Mat &img, std::map<std::string, float> emotions, cv::Point org, cv::Scalar fgcolor,
                                cv::Scalar bgcolor) {
    ...
    auto drawEmotion = [&](int n, std::string text, float value) {
        ...
        cv::Rect r(torg.x + textWidth, torg.y - textSize.height, size.width - 2 * padding.width - textWidth,
...
// HeadPoseVisualizer
HeadPoseVisualizer::HeadPoseVisualizer(float scale, cv::Scalar xAxisColor, cv::Scalar yAxisColor, cv::Scalar zAxisColor,
                                       int axisThickness) :
        xAxisColor(xAxisColor), yAxisColor(yAxisColor), zAxisColor(zAxisColor), axisThickness(axisThickness),
        scale(scale) {
}
...
//    system("clear");
//    std::cout << "[PITCH (UP/DOWN)]: " << pitch << std::endl;
//    std::cout << "[YAW (LEFT/RIGHT)]: " << yaw << std::endl;
//    std::cout << "[Roll (TILT)]: " << roll << std::endl;
...
void Visualizer::drawFace(cv::Mat &img, Face::Ptr f, bool drawEmotionBar) {
    ...
    if (f->isLandmarksEnabled()) {
        ...
        for (size_t i_lm = 0UL; i_lm < n_lm / 2; ++i_lm) {
            ...
            cv::circle(img, cv::Point(x_lm, y_lm), 1 + static_cast<int>(0.012 * f->_location.width),
