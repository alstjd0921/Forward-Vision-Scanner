        ...
        while (true) {
            ...
            for (size_t i = 0; i < prev_detection_results.size(); i++) {
                ...
                if (face->isHeadPoseEnabled()) {
                    ...
                    double yaw = face->getHeadPose().angle_y;
                    double pitch = face->getHeadPose().angle_p;
                    pitch *= CV_PI / 180.0;
                    yaw *= CV_PI / 180.0;

                    if ((-0.4 < pitch && pitch < 0.6) && (-0.6 < yaw && yaw < 0.6)) {
                        focused++;
                    }
...
//                    << 1000.f / (timer["total"].getSmoothedDuration()) << " fps" ;
                out << "Number of focused: " << focused << ", Number of detected faces: "
                    << prev_detection_results.size();
        ...
        slog::info << "Total image throughput: " << framesCounter * (1000.f / timer["total"].getTotalDuration())
                   << " fps" << slog::endl;
